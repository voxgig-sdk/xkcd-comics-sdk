...
class XkcdComicsRetryFeature extends XkcdComicsBaseFeature
{
    ...
    public function init(XkcdComicsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (XkcdComicsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(XkcdComicsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(XkcdComicsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
