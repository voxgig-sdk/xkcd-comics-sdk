...
boru-driven command-line client **and** interactive REPL for the XkcdComics
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/xkcd-comics-cli)
make build

# 2. See usage (words, entities, env vars)
./xkcd-comics-cli --help

# 3. Provide credentials once, via the environment
export XKCD_COMICS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./xkcd-comics-cli load 1 info0            # {id:1} shorthand
./xkcd-comics-cli load '{id:1}' info0       # explicit match map

# 5. Override the API base URL for a single call
XKCD_COMICS_BASE=https://api.example.com ./xkcd-comics-cli load 1 info0

# 6. No arguments -> interactive REPL
./xkcd-comics-cli
xkcd-comics> load 1 info0
xkcd-comics> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/xkcd-comics-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/xkcd-comics-cli load 1 info0
   ```
...
```sh
./xkcd-comics-cli load 1 info0          # scalar shorthand for {id:1}
./xkcd-comics-cli load '{id:1}' info0     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `xkcd-comics>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `info0`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/xkcd-comics-cli`. |
...
(`./xkcd-comics-cli <expr>`) and interactively (the REPL), and expressions compose the
