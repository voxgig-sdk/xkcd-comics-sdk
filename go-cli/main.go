// Command XkcdComics-cli is a CLI + REPL for the XkcdComics SDK,
...
const prompt = "xkcd-comics"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "info0"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      XKCD_COMICS_APIKEY (api key), XKCD_COMICS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: XKCD_COMICS_APIKEY carries the API key and
	// XKCD_COMICS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("XKCD_COMICS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("XKCD_COMICS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewXkcdComicsSDK(opts)
