...
[MCP](https://modelcontextprotocol.io) server exposing the XkcdComics SDK as
two agent tools — `xkcd-comics_list` and `xkcd-comics_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/xkcd-comics-mcp)
make build

# 2. Provide credentials via the environment
export XKCD_COMICS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user xkcd-comics \
  -- /absolute/path/to/xkcd-comics-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./xkcd-comics-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// xkcd-comics_list: first page of records
{ "entity": "info0" }
{ "entity": "info0", "query": { } }

// xkcd-comics_load: one record by id
{ "entity": "info0", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/xkcd-comics-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user xkcd-comics \
     -- "$PWD"/dist/*/xkcd-comics-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `xkcd-comics_list` and `xkcd-comics_load` tools now appear
   in new sessions. Ask the agent to *"list info0 using xkcd-comics"*
   and it calls `xkcd-comics_list` with `{"entity":"info0"}`.
...
```sh
export XKCD_COMICS_APIKEY=sk_live_xxx            # API key
export XKCD_COMICS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `xkcd-comics_list` tool
...
### Call the `xkcd-comics_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `xkcd-comics_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `xkcd-comics_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `XKCD_COMICS_APIKEY` | API key sent with every request. |
| `XKCD_COMICS_BASE` | Optional override of the API base URL. |
...
```sh
./xkcd-comics-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"xkcd-comics_load","arguments":{"entity":"info0","query":{"id":1}}}}'
```
