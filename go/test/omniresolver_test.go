...
import (
	...
	omni "github.com/voxgig-sdk/xkcd-comics-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.XkcdComicsError); is {
				out := map[string]any{
					"name":    "XkcdComicsError",
...
func omniCtx(arg any, client *sdk.XkcdComicsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.XkcdComicsSDK); is {
			client = live
