...
local Info0Entity = {}
Info0Entity.__index = Info0Entity


function Info0Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, Info0Entity)
  self._name = "info0"
  self._client = client
...
function Info0Entity:get_name()
  return self._name
end


function Info0Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return Info0Entity.new(self._client, opts)
end
...
function Info0Entity:mark_deleted()
  self._deleted = true
end


function Info0Entity:deleted()
  return true == self._deleted
end


function Info0Entity:data_set(args)
  if args ~= nil then
...
function Info0Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function Info0Entity:match_set(args)
  if args ~= nil then
...
function Info0Entity:match_get()
...
function Info0Entity:stream(action, args, callopts)
...
---@param reqmatch Info0LoadMatch
---@param ctrl? table
---@return Info0
---@return string? err
function Info0Entity:load(reqmatch, ctrl)
  local utility = self._utility
...
function Info0Entity:_run_op(ctx, post_done)
  local utility = self._utility
