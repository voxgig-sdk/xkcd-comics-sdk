...
local XkcdComicsSDK = {}
XkcdComicsSDK.__index = XkcdComicsSDK
...
XkcdComicsSDK._make_feature = _make_feature


function XkcdComicsSDK.new(options)
  local self = setmetatable({}, XkcdComicsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function XkcdComicsSDK:options_map()
...
function XkcdComicsSDK:get_utility()
  return Utility.copy(self._utility)
end


function XkcdComicsSDK:get_root_ctx()
  return self._rootctx
end


function XkcdComicsSDK:prepare(fetchargs)
...
function XkcdComicsSDK:direct(fetchargs)
...
-- by passing it.
function XkcdComicsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function XkcdComicsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "XkcdComicsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function XkcdComicsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = XkcdComicsSDK.new(sdkopts)
  sdk.mode = "test"
