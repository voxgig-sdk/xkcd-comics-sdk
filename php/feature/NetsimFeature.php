...
class XkcdComicsNetsimFeature extends XkcdComicsBaseFeature
{
    ...
    private function _simulate(XkcdComicsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(XkcdComicsContext $ctx, array $applied): void
    {
        $client = $this->client;
