...
class FtCtrl extends XkcdComicsControl
{
...
class FtClient
{
    ...
    private XkcdComicsUtility $utility;

    public function __construct(XkcdComicsUtility $utility, array $options)
    {
    ...
    public function get_utility(): XkcdComicsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (XkcdComicsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public XkcdComicsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (XkcdComicsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = XkcdComicsConfig::make_feature($name);
    ...
    private static function buildUrl(XkcdComicsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof XkcdComicsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new XkcdComicsResponse($fetched);
        }
    ...
    private function fail(XkcdComicsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (XkcdComicsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
