...
{
    private static function utility(): XkcdComicsUtility
    {
        return new XkcdComicsUtility();
    }
    ...
    private static function ctx(array $over = []): XkcdComicsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new XkcdComicsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new XkcdComicsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof XkcdComicsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new XkcdComicsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = XkcdComicsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new XkcdComicsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = XkcdComicsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new XkcdComicsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = XkcdComicsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new XkcdComicsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = XkcdComicsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new XkcdComicsResponse([]),
            'result' => new XkcdComicsResult([]),
        ]);
        [, $err] = XkcdComicsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new XkcdComicsSpec([]),
            'response' => null,
            'result' => new XkcdComicsResult([]),
        ]);
        [, $err] = XkcdComicsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new XkcdComicsSpec(['step' => 's']),
            'response' => new XkcdComicsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new XkcdComicsResult([]),
        ]);
        [, $err] = XkcdComicsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new XkcdComicsSpec(['step' => 's']),
            'response' => new XkcdComicsResponse(self::resp(200, ['v' => 1])),
            'result' => new XkcdComicsResult([]),
        ]);
        [, $err] = XkcdComicsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new XkcdComicsSpec(['step' => 's']),
            'response' => new XkcdComicsResponse(self::resp(200, ['v' => 2])),
            'result' => new XkcdComicsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        XkcdComicsMakeResponse::call($ctx);
    ...
    {
        $preset = new XkcdComicsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new XkcdComicsSpec([]),
            'response' => new XkcdComicsResponse([]),
            'result' => new XkcdComicsResult([]),
        ...
        [$got, $err] = XkcdComicsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new XkcdComicsResult([])]);
        [, $err] = XkcdComicsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new XkcdComicsSpec([]), 'result' => null]);
        [, $err] = XkcdComicsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new XkcdComicsSpec(['step' => 's']),
            'result' => new XkcdComicsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new XkcdComicsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = XkcdComicsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new XkcdComicsSpec(['step' => 's']),
            'result' => new XkcdComicsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new XkcdComicsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = XkcdComicsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new XkcdComicsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new XkcdComicsSpec([]),
            'result' => new XkcdComicsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = XkcdComicsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = XkcdComicsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new XkcdComicsError('boom', 'boom');
        $utility->fetcher = function (XkcdComicsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new XkcdComicsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = XkcdComicsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (XkcdComicsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = XkcdComicsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(XkcdComicsResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        XkcdComicsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (XkcdComicsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new XkcdComicsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = XkcdComicsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new XkcdComicsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new XkcdComicsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = XkcdComicsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = XkcdComicsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = XkcdComicsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new XkcdComicsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, XkcdComicsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new XkcdComicsResult(['ok' => false])]);
        $this->expectException(XkcdComicsError::class);
        XkcdComicsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new XkcdComicsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', XkcdComicsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        XkcdComicsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new XkcdComicsBaseFeature();
        $b = new XkcdComicsBaseFeature();
        XkcdComicsFeatureAdd::call($ctx, $a);
        XkcdComicsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): XkcdComicsBaseFeature
    {
        $f = new XkcdComicsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        XkcdComicsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): XkcdComicsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new XkcdComicsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = XkcdComicsPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        XkcdComicsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new XkcdComicsResponse(['headers' => null]),
            'result' => new XkcdComicsResult([]),
        ]);
        XkcdComicsResultHeaders::call($ctx);
