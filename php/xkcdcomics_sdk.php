...
class XkcdComicsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = XkcdComicsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = XkcdComicsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!XkcdComicsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, XkcdComicsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return XkcdComicsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = XkcdComicsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = XkcdComicsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = XkcdComicsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new XkcdComicsError($op . "_allow",
                "XkcdComicsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = XkcdComicsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new XkcdComicsError("graphql_error",
                "XkcdComicsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Info0($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_info0 === null) {
                $this->_info0 = new Info0Entity($this, null);
            }
            return $this->_info0;
        }
        return new Info0Entity($this, $data);
    }
