...
class XkcdComicsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            XkcdComicsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = XkcdComicsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = XkcdComicsTestRunner.getenv("XKCD_COMICS_TEST_LIVE")
        override = XkcdComicsTestRunner.getenv("XKCD_COMICS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = XkcdComicsTestRunner.getenv(key)
        ...
        explain = XkcdComicsTestRunner.getenv("XKCD_COMICS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["XKCD_COMICS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if XkcdComicsTestRunner._test_control is not None:
            return XkcdComicsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                XkcdComicsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            XkcdComicsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = XkcdComicsTestRunner.load_test_control()
...
def load_env_local():
    XkcdComicsTestRunner.load_env_local()


def env_override(m):
    return XkcdComicsTestRunner.env_override(m)


def entity_data(v):
    return XkcdComicsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return XkcdComicsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return XkcdComicsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return XkcdComicsTestRunner.load_test_control()


def live_client_options():
    return XkcdComicsTestRunner.live_client_options()


def live_delay_ms():
    return XkcdComicsTestRunner.live_delay_ms()
