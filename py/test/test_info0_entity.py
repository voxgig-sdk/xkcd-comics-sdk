...
class TestInfo0Entity:

    def test_should_create_instance(self):
        testsdk = XkcdComicsSDK.test(None, None)
        ent = testsdk.Info0(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _info0_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "info0." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set XKCD_COMICS_TEST_INFO0_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        info0_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.info0")))
        info0_ref01_data = None
        if len(info0_ref01_data_raw) > 0:
...
def _info0_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/info0/Info0TestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("XKCD_COMICS_TEST_INFO0_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("XKCD_COMICS_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = XkcdComicsSDK(helpers.to_map(merged_opts))

    _live = env.get("XKCD_COMICS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("XKCD_COMICS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
