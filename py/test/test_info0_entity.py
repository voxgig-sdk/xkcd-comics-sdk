    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set XKCDCOMICS_TEST_INFO__ENTID JSON to run live")
        client = setup["client"]
...
def _info0_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("XKCDCOMICS_TEST_INFO__ENTID"))
    ...
    _live = env.get("XKCDCOMICS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("XKCDCOMICS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
