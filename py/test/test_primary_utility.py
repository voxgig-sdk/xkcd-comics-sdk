...
from xkcdcomics_sdk.core.response import XkcdComicsResponse
from xkcdcomics_sdk.core.operation import XkcdComicsOperation
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return XkcdComicsError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = XkcdComicsSDK.test(None, None)
        ...
        class TestInitFeature(XkcdComicsBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(XkcdComicsBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = XkcdComicsSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = XkcdComicsSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = XkcdComicsSDK.test(None, setup_opts)
