...
from xkcdcomics_sdk.core.helpers import get_ctx_prop, to_map
...
class XkcdComicsContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, XkcdComicsSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, XkcdComicsResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, XkcdComicsResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return XkcdComicsError(code, msg, self)
