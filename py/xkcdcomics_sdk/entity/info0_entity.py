...
from xkcdcomics_sdk.xkcdcomics_types import (
    Info0,
    Info0LoadMatch,
)
...
class Info0Entity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return Info0Entity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Info0:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Info0().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
