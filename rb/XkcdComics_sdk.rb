...
require_relative 'XkcdComics_types'
...
class XkcdComicsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = XkcdComicsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = XkcdComicsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, XkcdComicsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    XkcdComicsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = XkcdComicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = XkcdComicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = XkcdComicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = XkcdComicsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = XkcdComicsError.new(
        "graphql_error", "XkcdComicsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
