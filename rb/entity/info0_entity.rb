...
class Info0Entity
  ...
  def make
    opts = @_entopts.dup
    Info0Entity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = XkcdComicsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = XkcdComicsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Info0.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = XkcdComicsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
