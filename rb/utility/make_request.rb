...
module XkcdComicsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = XkcdComicsResponse.new({})
    ...
    if fetch_err
      ...
      response = XkcdComicsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = XkcdComicsResponse.new(fetched)
    else
