...
//     (\`new XkcdComicsSDK(...)\` / \`XkcdComicsSDK.test(...)\` become
//     \`XkcdComicsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"info0":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new XkcdComicsSDK(...)\` and any
// \`XkcdComicsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+XkcdComicsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/XkcdComicsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
