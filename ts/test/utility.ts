...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.XKCDCOMICS_TEST_LIVE ||
    'TRUE' === process.env.XKCDCOMICS_TEST_OVERRIDE
  ) {
  ...
  m.XKCDCOMICS_TEST_EXPLAIN = process.env.XKCDCOMICS_TEST_EXPLAIN || m.XKCDCOMICS_TEST_EXPLAIN
